...
and shows an end-to-end example of how to build a translation model.
  https://www.tensorflow.org/versions/master/tutorials/seq2seq_att/index.html
...
def attention_decoder(decoder_inputs,
  ...
  with variable_scope.variable_scope(
      scope or "attention_decoder") as scope:
    ...
    def attention(query):
      ...
      att = [] # Results of attention weights
      ...
      for a in xrange(num_heads):
        with variable_scope.variable_scope("Attention_%d" % a):
          ...
          d = math_ops.reduce_sum(
              array_ops.reshape(aw, [-1, attn_length, 1, 1]) * hidden,
              [1, 2])
          att.append(aw)
          ds.append(array_ops.reshape(d, [-1, attn_size]))
      return ds, tf.pack(att)

    outputs = []
    attention_weights = []
    prev = None
    batch_attn_size = array_ops.pack([batch_size, attn_size])
    attns = [array_ops.zeros(batch_attn_size)
             for _ in xrange(num_heads)]
    for a in attns:  # Ensure the second shape of attention vectors is set.
      a.set_shape([None, attn_size])
    if initial_state_attention:
      attns, att_weights = attention(initial_state)
    for i, inp in enumerate(decoder_inputs):
      if i > 0:
        variable_scope.get_variable_scope().reuse_variables()
      ...
      if i == 0 and initial_state_attention:
        with variable_scope.variable_scope(variable_scope.get_variable_scope(),
                                           reuse=True):
          attns, att_weights = attention(state)
      else:
        attns, att_weights = attention(state)
...
def embedding_attention_decoder(decoder_inputs,
  ...
  with variable_scope.variable_scope(
      scope or "embedding_attention_decoder") as scope:
...
def embedding_attention_seq2seq(encoder_inputs,
  ...
  """
  with variable_scope.variable_scope(
      scope or "embedding_attention_seq2seq") as scope:
    # Encoder.
    encoder_cell = rnn_cell.EmbeddingWrapper(
        cell, embedding_classes=num_encoder_symbols,
        embedding_size=embedding_size)
    ...
    if isinstance(feed_previous, bool):
      outputs, state, attention =  embedding_attention_decoder(
          ...
          feed_previous=feed_previous,
          initial_state_attention=initial_state_attention)
      return outputs, attention, state
    ...
    def decoder(feed_previous_bool):
      ...
      with variable_scope.variable_scope(
        ...
        if nest.is_sequence(state):
          state_list = nest.flatten(state)
        return outputs + attention + state_list 

    outputs_state_attention = control_flow_ops.cond(feed_previous,
                                              lambda: decoder(True),
                                              lambda: decoder(False))
    outputs_len = len(decoder_inputs)  # Outputs length same as decoder inputs.
    state_list = outputs_state_attention[2*outputs_len:]
    state = state_list[0]
    if nest.is_sequence(encoder_state):
      state = nest.pack_sequence_as(structure=encoder_state,
                                    flat_sequence=state_list)
    attention = outputs_state_attention[outputs_len:2*outputs_len]
    outputs = outputs_state_attention[:outputs_len]
...
def model_with_buckets(encoder_inputs, decoder_inputs, targets, weights,
                       buckets, seq2seq_att, softmax_loss_function=None,
                       per_example_loss=False, name=None):
  """Create a sequence-to-sequence model with support for bucketing.

  The seq2seq_att argument is a function that defines a sequence-to-sequence model,
  e.g., seq2seq_att = lambda x, y: basic_rnn_seq2seq(x, y, rnn_cell.GRUCell(24))

  Args:
    encoder_inputs: A list of Tensors to feed the encoder; first seq2seq_att input.
    decoder_inputs: A list of Tensors to feed the decoder; second seq2seq_att input.
    targets: A list of 1D batch-sized int32 Tensors (desired output sequence).
    weights: List of 1D batch-sized float-Tensors to weight the targets.
    buckets: A list of pairs of (input size, output size) for each bucket.
    seq2seq_att: A sequence-to-sequence model function; it takes 2 input that
  ...
  Returns:
    A tuple of the form (outputs, losses), where:
      outputs: The outputs for each bucket. Its j'th element consists of a list
        ...
        depending on the seq2seq_att model used.
  ...
  with ops.op_scope(all_inputs, name, "model_with_buckets", ):
    for j, bucket in enumerate(buckets):
      with variable_scope.variable_scope(variable_scope.get_variable_scope(),
        ...
        bucket_outputs, att, _ = seq2seq_att(encoder_inputs[:bucket[0]],
  ...
  return outputs, attention, losses
